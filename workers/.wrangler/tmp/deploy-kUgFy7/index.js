...
var UnauthorizedError = class extends AppError {
  constructor(message2 = "Unauthorized") {
    super(401, message2, "UNAUTHORIZED");
  }
};
__name(UnauthorizedError, "UnauthorizedError");
var ValidationError = class extends AppError {
  constructor(message2 = "Validation failed") {
    super(400, message2, "VALIDATION_ERROR");
  }
};
...
__name(now, "now");
function successResponse(data, messageOrStatus) {
...
function fail(c, e, status) {
  const err = e;
  const s = status || err.status || 500;
  const message2 = err.message || "Internal server error";
  return c.json({ success: false, error: message2, code: err.code }, s);
}
__name(fail, "fail");
function paginated(c, data, total, page, limit) {
  return c.json({ success: true, data: { items: data, total, page, limit } });
}
__name(paginated, "paginated");

// src/durable-objects/notification-do.ts
import { DurableObject } from "cloudflare:workers";
var NotificationDurableObject = class extends DurableObject {
  ...
  async handleWebSocket(request) {
    ...
    server.accept();
    const clientEntry = { ws: server, userId };
  ...
  async handleSend(request) {
    const { userId, title, message: message2, type, link } = await request.json();
    const clients = this.connections.get(userId) || [];
...
__name(NotificationDurableObject, "NotificationDurableObject");

// src/repositories/logs.ts
var LogRepository = class {
  constructor(db) {
    this.db = db;
  }
  async findByProject(projectId, level, search, page = 1, limit = 20) {
    ...
    const logs = await this.db.prepare(query).bind(...params, limit.toString(), offset.toString()).all();
  ...
  async create(data) {
    ...
    ).bind(id, data.user_id, data.project_id || null, data.action, data.details || null, now2).run();
    return await this.db.prepare("SELECT * FROM activity_logs WHERE id = ?").bind(id).first();
  }
  async getErrors(projectId, severity, status, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    ...
    query += " ORDER BY created_at DESC LIMIT ? OFFSET ?";
    const total = await this.db.prepare(countQuery).bind(...params).first();
    const errors = await this.db.prepare(query).bind(...params, limit.toString(), offset.toString()).all();
  ...
  async createError(data) {
    ...
    return await this.db.prepare("SELECT * FROM error_logs WHERE id = ?").bind(id).first();
  }
  async updateError(id, data) {
    ...
    if (data.stack_trace !== void 0) {
      fields.push("stack_trace = ?");
      values.push(data.stack_trace);
    }
    ...
    if (fields.length === 0) {
      return this.db.prepare("SELECT * FROM error_logs WHERE id = ?").bind(id).first();
    }
    fields.push("updated_at = ?");
    values.push((/* @__PURE__ */ new Date()).toISOString());
    values.push(id);
    await this.db.prepare(`UPDATE error_logs SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
    return this.db.prepare("SELECT * FROM error_logs WHERE id = ?").bind(id).first();
  }
  async incrementErrorCount(id) {
    await this.db.prepare("UPDATE error_logs SET count = count + 1, updated_at = ? WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), id).run();
  }
};
__name(LogRepository, "LogRepository");
...
async function createToken(userId, secret) {
  ...
  return new SignJWT({ sub: userId }).setProtectedHeader({ alg: ALGORITHM }).setIssuedAt().setExpirationTime("7d").sign(secretKey);
}
...
var UserRepository = class {
  ...
  async update(id, data) {
    ...
    if (data.auth_provider !== void 0) {
      fields.push("auth_provider = ?");
      values.push(data.auth_provider);
    }
    if (data.auth_provider_id !== void 0) {
    ...
    fields.push("updated_at = ?");
    values.push(data.updated_at || (/* @__PURE__ */ new Date()).toISOString());
    values.push(id);
...
// src/auth/oauth.ts
async function handleGoogleOAuth(code, env) {
  ...
  user.password_hash = void 0;
  return user;
}
__name(handleGoogleOAuth, "handleGoogleOAuth");
async function handleGitHubOAuth(code, env) {
...
async function generateUniqueUsername(repo, base) {
  let username = base;
  let attempt = 0;
...
var registerSchema = external_exports.object({
  username: external_exports.string().min(3).max(30).regex(/^[a-zA-Z0-9_]+$/),
...
var updateUserSchema = external_exports.object({
  ...
  username: external_exports.string().min(3).max(30).regex(/^[a-zA-Z0-9_]+$/).optional(),
  bio: external_exports.string().max(500).optional(),
  avatar_url: external_exports.string().url().optional().or(external_exports.literal("")),
  skills: external_exports.string().max(500).optional(),
  company: external_exports.string().max(100).optional(),
  experience: external_exports.string().max(500).optional(),
  website: external_exports.string().url().optional().or(external_exports.literal("")),
  github: external_exports.string().max(100).optional()
});
var createCompanySchema = external_exports.object({
  name: external_exports.string().min(1).max(100),
  description: external_exports.string().max(1e3).optional(),
  website: external_exports.string().url().optional().or(external_exports.literal("")),
  tech_stack: external_exports.string().max(500).optional()
});
var sendConnectionSchema = external_exports.object({
  receiver_id: external_exports.string().uuid()
});
var createProjectSchema = external_exports.object({
  name: external_exports.string().min(1).max(100),
  description: external_exports.string().max(2e3).optional(),
  repo_url: external_exports.string().url().optional().or(external_exports.literal("")),
  repo_provider: external_exports.string().max(50).optional(),
  branch: external_exports.string().max(100).optional(),
  framework: external_exports.string().max(50).optional(),
  build_command: external_exports.string().max(500).optional(),
  start_command: external_exports.string().max(500).optional(),
  environment: external_exports.string().max(500).optional(),
...
var updateProjectSchema = external_exports.object({
  ...
  repo_provider: external_exports.string().max(50).optional(),
  branch: external_exports.string().max(100).optional(),
  status: external_exports.enum(["creating", "building", "deployed", "failed", "stopped"]).optional(),
  framework: external_exports.string().max(50).optional(),
  build_command: external_exports.string().max(500).optional(),
  start_command: external_exports.string().max(500).optional(),
  environment: external_exports.string().max(500).optional(),
  tunnel_url: external_exports.string().optional()
});
var createTaskSchema = external_exports.object({
  title: external_exports.string().min(1).max(200),
  description: external_exports.string().max(5e3).optional(),
  assignee_id: external_exports.string().uuid().optional(),
  due_date: external_exports.string().datetime().optional(),
...
var updateTaskSchema = external_exports.object({
  ...
  description: external_exports.string().max(5e3).optional(),
  assignee_id: external_exports.string().uuid().nullable().optional(),
  due_date: external_exports.string().datetime().nullable().optional(),
  priority: external_exports.enum(["low", "medium", "high", "urgent"]).optional(),
  status: external_exports.enum(["todo", "in-progress", "testing", "done"]).optional()
});
var createBugSchema = external_exports.object({
  title: external_exports.string().min(1).max(200),
  description: external_exports.string().min(1).max(1e4),
  screenshot_url: external_exports.string().url().optional().or(external_exports.literal("")),
  video_url: external_exports.string().url().optional().or(external_exports.literal("")),
  priority: external_exports.enum(["low", "medium", "high", "critical"]).optional(),
  assigned_to: external_exports.string().uuid().optional()
});
var updateBugSchema = external_exports.object({
  title: external_exports.string().min(1).max(200).optional(),
  description: external_exports.string().min(1).max(1e4).optional(),
  screenshot_url: external_exports.string().url().optional().or(external_exports.literal("")),
  video_url: external_exports.string().url().optional().or(external_exports.literal("")),
  priority: external_exports.enum(["low", "medium", "high", "critical"]).optional(),
  status: external_exports.enum(["open", "in-progress", "testing", "fixed", "closed"]).optional(),
  assigned_to: external_exports.string().uuid().nullable().optional()
});
var createChatSchema = external_exports.object({
  type: external_exports.enum(["private", "group", "project"]),
  name: external_exports.string().min(1).max(100).optional(),
  project_id: external_exports.string().uuid().optional(),
  member_ids: external_exports.array(external_exports.string().uuid()).min(1)
});
var sendMessageSchema = external_exports.object({
  text: external_exports.string().min(1).max(1e4),
  file_url: external_exports.string().url().optional().or(external_exports.literal("")),
  file_type: external_exports.string().max(50).optional(),
...
var searchSchema = external_exports.object({
  ...
  type: external_exports.enum(["users", "projects", "tasks", "bugs", "all"]).optional().default("all"),
...
var runAITestSchema = external_exports.object({
  prompt: external_exports.string().min(1).max(5e3)
});
...
async function googleCallback(c) {
  ...
  try {
    ...
    const message2 = err instanceof Error ? err.message : "Google OAuth failed";
    return c.redirect(`${c.env.FRONTEND_URL}/auth/callback?error=${encodeURIComponent(message2)}`);
  }
}
__name(googleCallback, "googleCallback");
async function githubAuth(c) {
...
async function githubCallback(c) {
  ...
  try {
    ...
    const message2 = err instanceof Error ? err.message : "GitHub OAuth failed";
    return c.redirect(`${c.env.FRONTEND_URL}/auth/callback?error=${encodeURIComponent(message2)}`);
  }
}
__name(githubCallback, "githubCallback");
...
async function listUsers(c) {
  ...
  if (query) {
    ...
    const users2 = result.users.map((u) => {
      u.password_hash = void 0;
      return u;
    });
    return c.json(successResponse({ users: users2, total: result.total, page, limit }));
  }
  const offset = (page - 1) * limit;
  const usersResult = await c.env.DB.prepare("SELECT * FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(limit, offset).all();
...
router2.get("/", authenticate, listUsers);
router2.get("/:id", authenticate, getUser);
router2.patch("/:id", authenticate, updateUser);
router2.delete("/:id", authenticate, deleteUser);
router2.patch("/:id/avatar", authenticate, updateAvatar);
var users_default = router2;

// src/controllers/search.ts
async function globalSearch(c) {
  const query = c.req.query("q");
  const page = parseInt(c.req.query("page") || "1", 10);
  const limit = parseInt(c.req.query("limit") || "10", 10);
  const parsed = searchSchema.safeParse({ q: query, page, limit });
  if (!parsed.success) {
    throw new ValidationError(
      parsed.error.errors.map((e) => e.message).join(", ")
    );
...
router3.get("/", authenticate, globalSearch);
var search_default = router3;

// src/repositories/connection.ts
var ConnectionRepository = class {
  constructor(db) {
    this.db = db;
  }
  async findById(id) {
    return this.db.prepare("SELECT * FROM connections WHERE id = ?").bind(id).first();
  }
  async findByUsers(requesterId, receiverId) {
    return this.db.prepare(
      "SELECT * FROM connections WHERE (requester_id = ? AND receiver_id = ?) OR (requester_id = ? AND receiver_id = ?)"
    ).bind(requesterId, receiverId, receiverId, requesterId).first();
  }
  async findByUser(userId, status, page = 1, limit = 20) {
    ...
    const total = await this.db.prepare(countSql).bind(...params).first();
    const connections = await this.db.prepare(sql).bind(...params, limit.toString(), offset.toString()).all();
    return { connections: connections.results, total: total?.total ?? 0 };
  }
  async findPendingForUser(userId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM connections WHERE receiver_id = ? AND status = ?").bind(userId, "pending").first();
    const connections = await this.db.prepare("SELECT * FROM connections WHERE receiver_id = ? AND status = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(userId, "pending", limit.toString(), offset.toString()).all();
    return { connections: connections.results, total: total?.total ?? 0 };
  }
  async findSentByUser(userId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM connections WHERE requester_id = ? AND status = ?").bind(userId, "pending").first();
    const connections = await this.db.prepare("SELECT * FROM connections WHERE requester_id = ? AND status = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(userId, "pending", limit.toString(), offset.toString()).all();
    return { connections: connections.results, total: total?.total ?? 0 };
  }
  async create(data) {
    const id = generateId();
    ...
    ).bind(id, data.requester_id, data.receiver_id, "pending", timestamp, timestamp).run();
  ...
  async update(id, data) {
    const timestamp = now();
    const result = await this.db.prepare("UPDATE connections SET status = ?, updated_at = ? WHERE id = ?").bind(data.status, timestamp, id).run();
...
__name(ConnectionRepository, "ConnectionRepository");
...
async function acceptRequest(c) {
  ...
  if (connection.status !== "pending")
    throw new ValidationError("Connection is not pending");
...
router4.post("/", authenticate, sendRequest);
router4.patch("/accept", authenticate, acceptRequest);
router4.patch("/reject", authenticate, rejectRequest);
router4.delete("/:id", authenticate, removeConnection);
var connections_default = router4;
...
var CompanyRepository = class {
  ...
  async findById(id) {
    return this.db.prepare("SELECT * FROM companies WHERE id = ?").bind(id).first();
  }
  async findAll(page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM companies").first();
    const companies = await this.db.prepare("SELECT * FROM companies ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(limit.toString(), offset.toString()).all();
    return { companies: companies.results, total: total?.total ?? 0 };
  }
  async search(query, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const searchPattern = `%${query}%`;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM companies WHERE name LIKE ? OR description LIKE ? OR tech_stack LIKE ?").bind(searchPattern, searchPattern, searchPattern).first();
    ...
    ).bind(searchPattern, searchPattern, searchPattern, limit.toString(), offset.toString()).all();
    return { companies: companies.results, total: total?.total ?? 0 };
  }
  async create(data) {
    const id = generateId();
    ...
    ).bind(id, data.name, data.description ?? null, data.website ?? null, data.tech_stack, data.created_by, timestamp).run();
    const company = await this.findById(id);
    if (!company)
      throw new NotFoundError("Company not found after creation");
    return company;
  }
  async update(id, data) {
    ...
    const website = data.website !== void 0 ? data.website : existing.website;
    const tech_stack = data.tech_stack ?? existing.tech_stack;
    await this.db.prepare("UPDATE companies SET name = ?, description = ?, website = ?, tech_stack = ? WHERE id = ?").bind(name, description, website, tech_stack, id).run();
    const company = await this.findById(id);
    if (!company)
      throw new NotFoundError("Company not found");
    return company;
  ...
  async addMember(companyId, userId, role) {
    await this.db.prepare("INSERT INTO company_members (company_id, user_id, role, joined_at) VALUES (?, ?, ?, ?)").bind(companyId, userId, role, now()).run();
  }
  async removeMember(companyId, userId) {
    const result = await this.db.prepare("DELETE FROM company_members WHERE company_id = ? AND user_id = ?").bind(companyId, userId).run();
    if (result.meta.changes === 0)
...
__name(CompanyRepository, "CompanyRepository");

// src/controllers/companies.ts
...
async function listCompanies(c) {
  const repo = new CompanyRepository(c.env.DB);
  const page = parseInt(c.req.query("page") || "1", 10);
...
__name(listCompanies, "listCompanies");
async function getCompany(c) {
  const id = c.req.param("id");
  const repo = new CompanyRepository(c.env.DB);
...
__name(deleteCompany, "deleteCompany");
async function joinCompany(c) {
  const userId = c.get("userId");
  const id = c.req.param("id");
  const repo = new CompanyRepository(c.env.DB);
  const company = await repo.findById(id);
...
__name(joinCompany, "joinCompany");
async function leaveCompany(c) {
  const userId = c.get("userId");
  const id = c.req.param("id");
  const repo = new CompanyRepository(c.env.DB);
  const company = await repo.findById(id);
...
__name(leaveCompany, "leaveCompany");

// src/routes/companies.ts
var router5 = new Hono2();
router5.post("/", authenticate, createCompany);
router5.get("/", authenticate, listCompanies);
router5.get("/:id", authenticate, getCompany);
router5.patch("/:id", authenticate, updateCompany);
router5.delete("/:id", authenticate, deleteCompany);
router5.post("/:id/join", authenticate, joinCompany);
router5.post("/:id/leave", authenticate, leaveCompany);
...
var ProjectRepository = class {
  ...
  async findAll(userId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    if (userId) {
      const total2 = await this.db.prepare(
        "SELECT COUNT(*) as total FROM projects p JOIN project_members pm ON pm.project_id = p.id WHERE pm.user_id = ?"
      ).bind(userId).first();
      const projects2 = await this.db.prepare(
        `SELECT p.* FROM projects p
           JOIN project_members pm ON pm.project_id = p.id
           WHERE pm.user_id = ?
           ORDER BY p.created_at DESC LIMIT ? OFFSET ?`
      ).bind(userId, limit.toString(), offset.toString()).all();
      return { projects: projects2.results, total: total2?.total ?? 0 };
    }
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM projects").first();
    const projects = await this.db.prepare("SELECT * FROM projects ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(limit.toString(), offset.toString()).all();
  ...
  async update(id, data) {
    ...
    const name = data.name ?? existing.name;
    const description = data.description !== void 0 ? data.description : existing.description;
    const repo_url = data.repo_url !== void 0 ? data.repo_url : existing.repo_url;
    const repo_provider = data.repo_provider !== void 0 ? data.repo_provider : existing.repo_provider;
    const branch = data.branch ?? existing.branch;
    const status = data.status ?? existing.status;
    const framework = data.framework !== void 0 ? data.framework : existing.framework;
    const build_command = data.build_command !== void 0 ? data.build_command : existing.build_command;
    const start_command = data.start_command !== void 0 ? data.start_command : existing.start_command;
    const environment = data.environment !== void 0 ? data.environment : existing.environment;
    const tunnel_url = data.tunnel_url !== void 0 ? data.tunnel_url : existing.tunnel_url;
    await this.db.prepare(
  ...
  async addMember(projectId, userId, data) {
    ...
    ).bind(projectId, userId, data.role, data.permissions, now()).run();
  }
  async removeMember(projectId, userId) {
    const result = await this.db.prepare("DELETE FROM project_members WHERE project_id = ? AND user_id = ?").bind(projectId, userId).run();
    if (result.meta.changes === 0)
      throw new NotFoundError("Member not found");
  }
  ...
  async updateMember(projectId, userId, data) {
    ...
    const result = await this.db.prepare(`UPDATE project_members SET ${sets.join(", ")} WHERE project_id = ? AND user_id = ?`).bind(...params).run();
    if (result.meta.changes === 0)
      throw new NotFoundError("Member not found");
  }
};
__name(ProjectRepository, "ProjectRepository");
...
router6.post("/", authenticate, createProject);
router6.get("/", authenticate, listProjects);
router6.get("/:id", authenticate, getProject);
router6.patch("/:id", authenticate, updateProject);
router6.delete("/:id", authenticate, deleteProject);
router6.post("/:id/members", authenticate, addMember);
router6.delete("/:id/members/:userId", authenticate, removeMember);
router6.patch("/:id/members/:userId", authenticate, updateMember);
var projects_default = router6;

// src/repositories/deployment.ts
var DeploymentRepository = class {
  db;
  constructor(db) {
    this.db = db;
  }
  async findById(id) {
    const result = await this.db.prepare("SELECT * FROM deployments WHERE id = ?").bind(id).first();
    return result ?? null;
  }
  async findByProject(projectId, page = 1, limit = 10) {
    const offset = (page - 1) * limit;
    const countResult = await this.db.prepare("SELECT COUNT(*) as total FROM deployments WHERE project_id = ?").bind(projectId).first();
    const total = countResult?.total ?? 0;
    const deployments = await this.db.prepare(
  ...
  async update(id, data) {
    ...
    if (data.commit_hash !== void 0) {
      sets.push("commit_hash = ?");
      values.push(data.commit_hash);
    }
    if (data.commit_message !== void 0) {
    ...
    if (sets.length === 0)
      return;
    values.push(id);
    await this.db.prepare(`UPDATE deployments SET ${sets.join(", ")} WHERE id = ?`).bind(...values).run();
  }
...
__name(DeploymentRepository, "DeploymentRepository");

// src/controllers/deployments.ts
async function createDeployment(c) {
  try {
    const userId = c.get("userId");
    ...
    const nextVersion = latestDeployment ? incrementVersion(latestDeployment.version) : "v1.0.0";
    const deployment = await repo.create({
      id: crypto.randomUUID(),
      project_id,
      version: nextVersion,
      commit_hash: commit_hash ?? void 0,
      commit_message: commit_message ?? void 0,
      branch,
      status: "pending",
      logs: void 0,
      created_by: userId
...
async function listDeployments(c) {
  try {
...
async function getDeployment(c) {
  try {
    const id = c.req.param("id");
    const repo = new DeploymentRepository(c.env.DB);
...
async function updateDeployment(c) {
  try {
    const id = c.req.param("id");
    const repo = new DeploymentRepository(c.env.DB);
...
function incrementVersion(version) {
  ...
  if (!match2)
    return "v1.0.0";
  const [major, minor, patch] = [parseInt(match2[1]), parseInt(match2[2]), parseInt(match2[3])];
...
router7.post("/", authenticate, createDeployment);
router7.get("/", authenticate, listDeployments);
router7.get("/:id", authenticate, getDeployment);
router7.patch("/:id", authenticate, updateDeployment);
var deployments_default = router7;

// src/controllers/terminal.ts
...
__name(getWebSocket, "getWebSocket");
...
router8.post("/session", authenticate, createSession);
router8.get("/ws", authenticate, getWebSocket);
router8.get("/history", authenticate, getHistory);
var terminal_default = router8;

// src/controllers/tunnel.ts
async function getProject2(c, projectId) {
  const project = await c.env.DB.prepare("SELECT * FROM projects WHERE id = ?").bind(projectId).first();
  return project ?? null;
}
__name(getProject2, "getProject");
async function startTunnel(c) {
  try {
    const { projectId, port } = await c.req.json();
    if (!projectId) {
      return fail(c, new Error("projectId is required"), 400);
    }
    const project = await getProject2(c, projectId);
    if (!project) {
      throw new NotFoundError("Project not found");
    }
    const tunnelPort = port ?? 3e3;
    const tunnelSubdomain = `opendrap-${projectId.slice(0, 8)}`;
    const tunnelUrl = `https://${tunnelSubdomain}.trycloudflare.com`;
    await c.env.DB.prepare("UPDATE projects SET tunnel_url = ?, updated_at = ? WHERE id = ?").bind(tunnelUrl, (/* @__PURE__ */ new Date()).toISOString(), projectId).run();
    return success(c, {
...
async function stopTunnel(c) {
  try {
    ...
    await c.env.DB.prepare("UPDATE projects SET tunnel_url = NULL, updated_at = ? WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), projectId).run();
    return success(c, { tunnel_url: null });
  } catch (err) {
    return fail(c, err);
  }
}
__name(stopTunnel, "stopTunnel");
async function getTunnelStatus(c) {
  try {
    const projectId = c.req.query("projectId");
    if (!projectId) {
      return fail(c, new Error("projectId query parameter is required"), 400);
...
__name(getTunnelStatus, "getTunnelStatus");

// src/routes/tunnel.ts
var router9 = new Hono2();
router9.post("/start", authenticate, startTunnel);
router9.post("/stop", authenticate, stopTunnel);
router9.get("/status", authenticate, getTunnelStatus);
var tunnel_default = router9;

// src/repositories/chat.ts
var ChatRepository = class {
  constructor(db) {
    this.db = db;
  }
  async findById(id) {
    const result = await this.db.prepare("SELECT * FROM chats WHERE id = ?").bind(id).first();
    return result ?? null;
  }
  async findPrivateChat(user1Id, user2Id) {
    const result = await this.db.prepare(
      `SELECT c.* FROM chats c
       WHERE c.type = 'private'
       AND EXISTS (SELECT 1 FROM chat_members WHERE chat_id = c.id AND user_id = ?)
       AND EXISTS (SELECT 1 FROM chat_members WHERE chat_id = c.id AND user_id = ?)`
    ).bind(user1Id, user2Id).first();
    return result ?? null;
  }
  ...
  async create(data) {
    const chat = {
      ...
      project_id: data.project_id ?? void 0,
  ...
  async saveMessage(data) {
    const message2 = {
      ...
      text: data.text,
      file_url: data.file_url ?? void 0,
      file_type: data.file_type ?? void 0,
      file_name: data.file_name ?? void 0,
      created_at: now()
    };
    await this.db.prepare(
      "INSERT INTO messages (id, chat_id, sender_id, text, file_url, file_type, file_name, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(message2.id, message2.chat_id, message2.sender_id, message2.text, message2.file_url, message2.file_type, message2.file_name, message2.created_at).run();
    return message2;
  }
};
__name(ChatRepository, "ChatRepository");

// src/controllers/chat.ts
async function createChat(c) {
  try {
    const userId = c.get("userId");
    const body = await c.req.json();
    if (!body.type)
    ...
    await repo.addMember(chat.id, userId);
    if (body.member_ids) {
      for (const memberId of body.member_ids) {
        if (memberId !== userId) {
          await repo.addMember(chat.id, memberId);
        }
      }
    }
...
__name(getChat, "getChat");
async function getChatMessages(c) {
  try {
    const id = c.req.param("id");
    const repo = new ChatRepository(c.env.DB);
    const chat = await repo.findById(id);
...
async function sendMessage(c) {
  try {
    ...
    if (!chat)
      throw new NotFoundError("Chat");
    const message2 = await repo.saveMessage({
      chat_id: chatId,
      sender_id: userId,
      text: body.text.trim(),
    ...
    return success(c, message2, 201);
  } catch (e) {
    return fail(c, e);
  }
}
__name(sendMessage, "sendMessage");
async function chatWebSocket(c) {
  try {
    const chatId = c.req.param("id");
    const doId = c.env.CHAT_DO.idFromName(chatId);
    const stub = c.env.CHAT_DO.get(doId);
...
__name(chatWebSocket, "chatWebSocket");
...
var TaskRepository = class {
  ...
  async findByProject(projectId, status, priority, assigneeId, page = 1, limit = 20) {
  ...
  async create(data) {
    const task = {
      ...
      assignee_id: data.assignee_id ?? void 0,
      due_date: data.due_date ?? void 0,
...
__name(TaskRepository, "TaskRepository");
...
async function addTaskComment(c) {
  try {
    const userId = c.get("userId");
...
async function getTaskComments(c) {
  try {
    const taskId = c.req.param("id");
    const repo = new TaskRepository(c.env.DB);
    const task = await repo.findById(taskId);
...
__name(getTaskComments, "getTaskComments");
...
// src/repositories/bug.ts
var BugRepository = class {
  constructor(db) {
    this.db = db;
  }
  async findById(id) {
    const result = await this.db.prepare("SELECT * FROM bugs WHERE id = ?").bind(id).first();
    return result ?? null;
  }
  async findByProject(projectId, status, priority, page = 1, limit = 20) {
    const conditions = ["project_id = ?"];
    const params = [projectId];
    if (status) {
      conditions.push("status = ?");
  ...
  async create(data) {
    const bug = {
      ...
      screenshot_url: data.screenshot_url ?? void 0,
      video_url: data.video_url ?? void 0,
      priority: data.priority,
      status: data.status,
      assigned_to: data.assigned_to ?? void 0,
      created_by: data.created_by,
      created_at: now(),
      updated_at: now()
    };
  ...
  async update(id, data) {
    ...
    if (data.screenshot_url !== void 0) {
      fields.push("screenshot_url = ?");
      params.push(data.screenshot_url);
    }
    if (data.video_url !== void 0) {
...
__name(BugRepository, "BugRepository");

// src/controllers/bugs.ts
async function reportBug(c) {
  try {
    const userId = c.get("userId");
    const body = await c.req.json();
    if (!body.project_id)
      throw new ValidationError("project_id is required");
    if (!body.title?.trim())
      throw new ValidationError("Bug title is required");
    if (!body.description?.trim())
      throw new ValidationError("Bug description is required");
...
__name(listBugs, "listBugs");
async function getBug(c) {
  try {
    const id = c.req.param("id");
    const repo = new BugRepository(c.env.DB);
...
__name(getBug, "getBug");
async function updateBug(c) {
  try {
    const id = c.req.param("id");
    const body = await c.req.json();
    const repo = new BugRepository(c.env.DB);
...
__name(updateBug, "updateBug");
async function deleteBug(c) {
  try {
    const id = c.req.param("id");
    const repo = new BugRepository(c.env.DB);
    const bug = await repo.findById(id);
...
__name(deleteBug, "deleteBug");
async function addBugComment(c) {
  try {
    const userId = c.get("userId");
...
__name(addBugComment, "addBugComment");
async function getBugComments(c) {
  try {
    const bugId = c.req.param("id");
    const repo = new BugRepository(c.env.DB);
    const bug = await repo.findById(bugId);
...
__name(getBugComments, "getBugComments");

// src/routes/bugs.ts
var bugsRouter = new Hono2();
...
__name(listErrors, "listErrors");
async function updateErrorStatus(c) {
  const env = c.env;
  const repo = new LogRepository(env.DB);
  const id = c.req.param("id");
...
// src/ai/index.ts
var AIService = class {
  async analyzeLog(logData, apiKey, apiUrl) {
...
Respond in JSON format with keys: rootCause, suggestedFix, confidenceScore`;
  ...
  async analyzeBug(bugData, apiKey, apiUrl) {
...
Respond in JSON format with keys: summary, possibleCauses, suggestedFix, priority`;
  ...
  async generateTests(prompt, apiKey, apiUrl) {
...
Respond in JSON format with keys: testCases (array of objects with keys: name, steps, expectedResults)`;
  ...
  async callGLM(prompt, apiKey, apiUrl) {
...
// src/repositories/ai.ts
var AITestRepository = class {
  constructor(db) {
    this.db = db;
  }
  async findByProject(projectId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM ai_test_runs WHERE project_id = ?").bind(projectId).first();
    const runs = await this.db.prepare("SELECT * FROM ai_test_runs WHERE project_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(projectId, limit.toString(), offset.toString()).all();
    return { runs: runs.results, total: total?.total ?? 0 };
  }
  async findById(id) {
    return this.db.prepare("SELECT * FROM ai_test_runs WHERE id = ?").bind(id).first();
  }
  async create(data) {
    const id = crypto.randomUUID();
    const now2 = (/* @__PURE__ */ new Date()).toISOString();
    ...
    ).bind(id, data.project_id, data.prompt, "pending", null, data.created_by, now2).run();
  ...
  async update(id, data) {
    ...
    if (fields.length === 0)
      return this.findById(id);
    values.push(id);
    await this.db.prepare(`UPDATE ai_test_runs SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
    return this.findById(id);
  }
};
__name(AITestRepository, "AITestRepository");

// src/controllers/ai.ts
async function analyzeLog(c) {
  const env = c.env;
...
__name(analyzeLog, "analyzeLog");
async function analyzeBug(c) {
  const env = c.env;
...
__name(analyzeBug, "analyzeBug");
async function generateTests(c) {
  const env = c.env;
...
__name(generateTests, "generateTests");
async function runTest(c) {
  ...
  try {
    ...
    await repo.update(run.id, { status: "failed", results: err.message });
    const updated = await repo.findById(run.id);
...
var TestRepository = class {
  ...
  async findById(id) {
    return this.db.prepare("SELECT * FROM test_runs WHERE id = ?").bind(id).first();
  }
  async findByProject(projectId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM test_runs WHERE project_id = ?").bind(projectId).first();
    const runs = await this.db.prepare("SELECT * FROM test_runs WHERE project_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(projectId, limit.toString(), offset.toString()).all();
    return { runs: runs.results, total: total?.total ?? 0 };
  }
  async create(data) {
    const id = crypto.randomUUID();
    ...
    ).bind(id, data.project_id, "pending", data.config || null, data.created_by, now2, now2).run();
    return await this.findById(id);
  }
  async update(id, data) {
    const fields = [];
    ...
    if (data.test_results !== void 0) {
      fields.push("test_results = ?");
      values.push(data.test_results);
    }
    if (data.screenshots !== void 0) {
    ...
    values.push((/* @__PURE__ */ new Date()).toISOString());
    values.push(id);
    await this.db.prepare(`UPDATE test_runs SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
    return this.findById(id);
  }
};
__name(TestRepository, "TestRepository");

// src/controllers/tests.ts
async function triggerTestRun(c) {
  const env = c.env;
  const userId = c.get("userId");
...
__name(triggerTestRun, "triggerTestRun");
async function getTestRun(c) {
  const env = c.env;
  const id = c.req.param("id");
  const repo = new TestRepository(env.DB);
...
var NotificationRepository = class {
  ...
  async findByUser(userId, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const total = await this.db.prepare("SELECT COUNT(*) as total FROM notifications WHERE user_id = ?").bind(userId).first();
    const notifications = await this.db.prepare("SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(userId, limit.toString(), offset.toString()).all();
  ...
  async create(data) {
    ...
    ).bind(id, data.user_id, data.type, data.title, data.message, data.link || null, 0, now2).run();
    return await this.db.prepare("SELECT * FROM notifications WHERE id = ?").bind(id).first();
  }
  async markRead(id) {
  ...
  async getUnreadCount(userId) {
    const result = await this.db.prepare("SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND read = 0").bind(userId).first();
...
async function markAllAsRead(c) {
  const env = c.env;
  const userId = c.get("userId");
  const repo = new NotificationRepository(env.DB);
...
async function websocketHandler(c) {
  const env = c.env;
  const userId = c.get("userId");
...
var ADMIN_KEY = "opendrapdev@2026";
...
var VALID_PLANS = ["free", "pro", "enterprise"];
...
async function getStats(c) {
  ...
  const [users, disabled, projects, deployments, openBugs, companies, planRows] = await Promise.all([
    ...
    db.prepare("SELECT COUNT(*) as n FROM users WHERE is_disabled=1").first(),
    db.prepare("SELECT COUNT(*) as n FROM projects").first(),
    db.prepare("SELECT COUNT(*) as n FROM deployments").first(),
    db.prepare("SELECT COUNT(*) as n FROM bugs WHERE status='open'").first(),
...
async function listAllUsers(c) {
  ...
  if (q) {
    ...
    const [rows2, count2] = await Promise.all([
      db.prepare(SELECT + WHERE + " ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(p, p, p, limit, offset).all(),
      db.prepare("SELECT COUNT(*) as n FROM users" + WHERE).bind(p, p, p).first()
    ]);
    return c.json(successResponse({ users: rows2.results, total: count2?.n ?? 0, page, limit }));
  }
...
async function getUserDetail(c) {
  ...
  const [user, projectCount, deployCount] = await Promise.all([
    ...
    db.prepare("SELECT COUNT(*) as n FROM projects WHERE created_by=?").bind(id).first(),
    db.prepare("SELECT COUNT(*) as n FROM deployments WHERE created_by=?").bind(id).first()
  ]);
  if (!user)
...
async function updateUserAdmin(c) {
  const id = c.req.param("id");
  const db = c.env.DB;
  const body = await c.req.json();
  const fields = [];
...
__name(updateUserAdmin, "updateUserAdmin");
async function deleteUserAdmin(c) {
  const id = c.req.param("id");
  await c.env.DB.prepare("DELETE FROM users WHERE id=?").bind(id).run();
  return c.json(successResponse({ message: "User deleted" }));
}
__name(deleteUserAdmin, "deleteUserAdmin");
async function listEnvVars(c) {
  const rows = await c.env.DB.prepare(
    "SELECT * FROM env_vars ORDER BY key ASC"
  ).all();
  return c.json(successResponse(rows.results));
}
__name(listEnvVars, "listEnvVars");
async function upsertEnvVar(c) {
  ...
  const now2 = (/* @__PURE__ */ new Date()).toISOString();
  const existing = await c.env.DB.prepare("SELECT id FROM env_vars WHERE key=?").bind(key).first();
  if (existing) {
    await c.env.DB.prepare(
      "UPDATE env_vars SET value=?, description=?, is_secret=?, updated_at=? WHERE key=?"
    ).bind(value, description, is_secret, now2, key).run();
  } else {
    await c.env.DB.prepare(
      "INSERT INTO env_vars (id,key,value,description,is_secret,created_at,updated_at) VALUES (?,?,?,?,?,?,?)"
    ).bind(generateId(), key, value, description, is_secret, now2, now2).run();
  }
  const row = await c.env.DB.prepare("SELECT * FROM env_vars WHERE key=?").bind(key).first();
  return c.json(successResponse(row));
}
__name(upsertEnvVar, "upsertEnvVar");
async function deleteEnvVar(c) {
  const key = decodeURIComponent(c.req.param("key"));
  await c.env.DB.prepare("DELETE FROM env_vars WHERE key=?").bind(key).run();
  return c.json(successResponse({ message: "Deleted" }));
}
__name(deleteEnvVar, "deleteEnvVar");

// src/routes/admin.ts
var router10 = new Hono2();
router10.use("*", adminAuth);
router10.get("/stats", getStats);
router10.get("/users", listAllUsers);
router10.get("/users/:id", getUserDetail);
router10.patch("/users/:id", updateUserAdmin);
router10.delete("/users/:id", deleteUserAdmin);
router10.get("/env", listEnvVars);
router10.put("/env/:key", upsertEnvVar);
router10.delete("/env/:key", deleteEnvVar);
var admin_default = router10;

// src/controllers/agent.ts
async function handleAgentWebSocket(c) {
  try {
...
__name(handleAgentWebSocket, "handleAgentWebSocket");
async function handleAgentCommand(c) {
  try {
    const projectId = c.req.param("projectId");
    if (!projectId) {
      return fail(c, new Error("projectId is required"), 400);
...
async function handleAgentStatus(c) {
  try {
    const projectId = c.req.param("projectId");
    if (!projectId) {
      return fail(c, new Error("projectId is required"), 400);
...
__name(handleAgentStatus, "handleAgentStatus");

// src/routes/agent.ts
var router11 = new Hono2();
router11.get("/:projectId/ws", authenticate, handleAgentWebSocket);
router11.post("/:projectId/command", authenticate, handleAgentCommand);
router11.get("/:projectId/status", authenticate, handleAgentStatus);
var agent_default = router11;
...
async function getDashboard(c) {
  ...
  const db = env.DB;
  const projectCount = await db.prepare("SELECT COUNT(*) as count FROM projects WHERE created_by = ?").bind(user.id).first();
  const deploymentCount = await db.prepare("SELECT COUNT(*) as count FROM deployments WHERE created_by = ?").bind(user.id).first();
  const errorCount = await db.prepare("SELECT COUNT(*) as count FROM error_logs").first();
  const bugCount = await db.prepare("SELECT COUNT(*) as count FROM bugs WHERE assigned_to = ?").bind(user.id).first();
...
// src/durable-objects/chat-do.ts
var ChatDurableObject = class {
  ...
  constructor(ctx, env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      const stored = await ctx.storage.get("chatState");
      if (stored) {
  ...
  async fetch(request) {
    ...
    if (method === "POST" && url.pathname === "/message") {
      ...
      this.chatState.messages.push(message2);
      await this.state.storage.put("chatState", this.chatState);
      this.broadcast({ type: "new_message", message: message2 });
      return new Response(JSON.stringify({ success: true, data: message2 }), {
        headers: { "Content-Type": "application/json" }
      });
    }
    return new Response("Not Found", { status: 404 });
...
__name(ChatDurableObject, "ChatDurableObject");

// src/durable-objects/agent.ts
var AgentDurableObject = class {
  state;
  appEnv;
  sessionState;
    ...
    switch (type) {
      ...
      case "heartbeat": {
        if (this.sessionState.session) {
          this.sessionState.session.last_heartbeat = (/* @__PURE__ */ new Date()).toISOString();
  ...
  handleAgentDisconnected() {
    ...
    if (ws) {
      try {
        ws.close(1e3, "Agent disconnected");
      } catch {
      }
      this.sessionState.webSocket = null;
    }
  }
  async handleQueueCommand(request) {
    try {
      const body = await request.json();
      if (!body.command) {
  ...
  broadcastLog(projectId, log) {
    const ws = this.sessionState.webSocket;
    if (ws) {
      ws.send(JSON.stringify({ type: "log_received", project_id: projectId, log }));
    }
  }
  storeCommandResult(sessionId, commandId, output, status) {
...
__name(AgentDurableObject, "AgentDurableObject");

// src/durable-objects/terminal.ts
var AGENT_SESSION_ID = "agent";
var HEARTBEAT_TIMEOUT_MS = 9e4;
var TerminalDurableObject = class {
  state;
  appEnv;
  sessions;
  websockets;
  ...
  constructor(state, env) {
    ...
    this.state.blockConcurrencyWhile(async () => {
      const stored = await this.state.storage.get("agentSessionId");
      const lastHb = await this.state.storage.get("lastHeartbeat");
      if (stored) {
        this.agentSessionId = stored;
        if (lastHb && Date.now() - lastHb < 18e4) {
  ...
  async handleWebSocketUpgrade(request) {
    ...
    server.send(JSON.stringify({
      ...
      hostname: this.agentSessionId ? "cloudshell" : void 0,
      cwd: this.agentSessionId ? "~" : void 0
    }));
    return new Response(null, { status: 101, webSocket: client });
  }
  handleDisconnect(sessionId, ws) {
  ...
  handleMessage(sessionId, projectId, data, ws) {
    const type = data.type;
    switch (type) {
      case "agent_connected": {
        ...
        const agentInfo = data.info || {};
        const hostname = agentInfo.hostname || "cloudshell";
        this.broadcastToBrowsers(JSON.stringify({
      ...
      case "terminal_input": {
        ...
        if (session) {
          ...
          if (session.buffer.length > 1e3) {
            session.buffer.splice(0, session.buffer.length - 1e3);
          }
        }
      ...
      case "command_output": {
        ...
        if (filtered.trim()) {
          for (const [sid, ws2] of this.websockets.entries()) {
            if (sid === AGENT_SESSION_ID)
              continue;
            const s = this.sessions.get(sid);
            if (s) {
              s.buffer.push(filtered);
              if (s.buffer.length > 1e3)
                s.buffer.splice(0, s.buffer.length - 1e3);
            }
            try {
              ws2.send(JSON.stringify({ type: "terminal_output", data: filtered }));
            } catch {
            }
          }
        }
      ...
      case "stop_process": {
        if (this.agentSessionId) {
          ...
          if (agentWs) {
            try {
              agentWs.send(JSON.stringify({ type: "stop_process", process_id: data.process_id }));
            } catch {
              this.handleAgentOffline(this.agentSessionId);
            }
          }
  ...
  broadcastToBrowsers(message2, excludeSessionId) {
...
__name(TerminalDurableObject, "TerminalDurableObject");
...
app.route("/api/connections", connections_default);
app.route("/api/companies", companies_default);
app.route("/api/projects", projects_default);
app.route("/api/chat", chatRouter);
app.route("/api/tasks", tasksRouter);
app.route("/api/bugs", bugsRouter);
app.route("/api/deployments", deployments_default);
app.route("/api/terminal", terminal_default);
app.route("/api/tunnel", tunnel_default);
...
app.route("/api/agent", agent_default);
app.route("/api", dashboardRouter);
app.get("/api/install.sh", (c) => {
  const queryToken = c.req.query("token") || "";
  const tokenLine = queryToken ? 'OPENDRAP_AGENT_TOKEN="' + queryToken.replace(/[\\"$]/g, "\\$&") + '"' : "";
...
' });
  ...
  const onChunk = (chunk) => {
    ...
    const cwdM = text.match(/__CWD__:([^\r
]+)/);
    if (cwdM) {
